...
function MainPage() {
		...
		async function checkAuthAndRedirect() {
			try {
				...
				if (user && session) {
					...
					// User is not authenticated, stay on this page to show login UI
					setIsLoading(false);
				}
			} catch (error) {
				console.error("Error checking authentication:", error);
				// Handle error, maybe show an error message or stay on login
				setIsLoading(false);
			}
		}
	...
	if (isLoading) {
		return <LoadingSpinner />; // Or any other suitable loading indicator
	}

	// Render the login UI (adapted from Auth.tsx)
	return (
		<div className="min-h-screen bg-gray-900 text-white">
			<div className="relative z-10 flex min-h-screen flex-col items-center justify-center p-4 md:p-8">
				...
				<main className="flex w-full max-w-6xl flex-col items-center justify-between gap-8 md:gap-16">
					...
					<Login /> {/* Login component for actual login actions */}
				</main>
